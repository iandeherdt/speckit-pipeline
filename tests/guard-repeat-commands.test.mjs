#!/usr/bin/env node
// Smoke test for files/scripts/guard-repeat-commands.mjs.
// Spawns the hook with synthetic PreToolUse payloads and asserts exit codes.
// Run with: `node tests/guard-repeat-commands.test.mjs`
...
const HOOK = join(dirname(fileURLToPath(import.meta.url)), '..', 'files', 'scripts', 'guard-repeat-commands.mjs');
...
// ── Case 6: SPECKIT_GUARD=0 bypass ──
{
  const cwd = makeCwd();
  writeTrace(cwd, 'abcd1234', [
    evt('2026-05-14T20:30:00.000Z', 'Bash', { input: { command: 'npm run typecheck' } }),
  ]);
  const r = runHook(cwd, 'abcd1234', 'npm run typecheck', { SPECKIT_GUARD: '0' });
  assert(r.status === 0, 'SPECKIT_GUARD=0 bypasses the guard');
  rmSync(cwd, { recursive: true, force: true });
}
